...
export const publicRouter = createTRPCRouter({
  ...
  addEventToBookmarks: protectedProcedure.input(z.string()).mutation(({ input, ctx }) => {
  ...
  removeEventFromBookmarks: protectedProcedure.input(z.string()).mutation(({ input, ctx }) =>
  ...
  getFavoriteAccounts: protectedProcedure.input(accountType).query(async ({ ctx, input }) => {
    const user = await ctx.prisma.user.findUnique({
      ...
      select: {
        ...
        favoriteMusicians:
          input.accountType === "musicians" || input.accountType === "performers"
            ? {
                select: {
                  id: true,
                },
